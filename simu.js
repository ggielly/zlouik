...
var simu = function() {
    ...
    function calculerPrelevementSociaux(MontantRevenuImposable, tranches, taux) {
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // Définition des tranches d'imposition et des taux correspondants
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    function calculerMontant_impotdividende(AC24, seuils, pourcentages) {
    ...
    // Définition des tranches d'imposition et des taux correspondants
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    function calculer_IR_EURL(AC16, tranche01_bas, tranche02_bas, tranche03_bas, tranche04_bas, tranche01_haut, tranche02_haut, tranche03_haut, tranche01_pourcent, tranche02_pourcent, tranche03_pourcent, tranche04_pourcent) {
    ...
    function creerMenuVillesDepart() {
    ...
    /**
    * Génère un tableau HTML affichant les résultats des trajets.
    * @param {Array} resultats Les résultats des trajets.
    * @param {number} prkVoiture Le PRK par kilomètre.
    * @param {number} prime_montant_01 La prime de match.
    * @returns {string} Le code HTML du tableau des résultats.
    */
    /*function genererTableauResultats(resultats, prkVoiture) {
        var totalPRK = 0;
        ...
        var indemniteChoisie = prime_montant_01; // Valeur par défaut de l'indemnité choisie
        ...
        var prk = 0;
        if (trajet.Km > 0) {
            prk = trajet.Km * parseFloat(prkVoiture);
        }

        var grandDeplacementSemaine;
        var indemniteKilometrique;
        var nombreRepas;
        var prixHotel;

        // Calcul du grand déplacement
        if (trajet.Km > 500) {
            grandDeplacementSemaine = 80; // Utiliser un nombre au lieu d'une chaîne pour les calculs
            nombreRepas = 2;
            prixHotel = 87;
        } else {
            grandDeplacementSemaine = 0; // Utiliser un nombre au lieu d'une chaîne pour les calculs
            nombreRepas = 1;
            prixHotel = 0;
        }

        // Calcul des indemnités kilométriques
        indemniteKilometrique = (trajet.Km * 0.410).toFixed(2);

        // Ajout d'une ligne au tableau pour chaque trajet
        tableauHtml += "<tr>";
        tableauHtml += "<td>" + trajet.VilleDepart + "</td>";
        tableauHtml += "<td>" + trajet.VilleDestination + "</td>";
        tableauHtml += "<td>" + trajet.Km + " Km</td>";
        tableauHtml += "<td>" + trajet.Peages + " €</td>";
        tableauHtml += "<td>" + trajet.TempsTrajet + "</td>";
        tableauHtml += "<td>" + grandDeplacementSemaine + "</td>";
        tableauHtml += "<td>" + indemniteKilometrique + " €</td>";
        tableauHtml += "<td>" + (nombreRepas * 17) + " €</td>";
        tableauHtml += "<td>" + prixHotel + " €</td>";
        tableauHtml += "<td>" + indemniteChoisie + " €</td>";
        tableauHtml += "<td>" + parseFloat(prk).toFixed(2) + " €</td>";
        tableauHtml += "</tr>";

        totalIndemnitesKilometriques += parseFloat(indemniteKilometrique);
        totalRepas += nombreRepas * 17;
        totalHotels += parseFloat(prixHotel);
        totalGrandDeplacement += parseFloat(grandDeplacementSemaine);
        totalDistance += parseFloat(trajet.Km);
        totalPeages += parseFloat(trajet.Peages);
        totalPRK += parseFloat(prk);

        // Extraction des heures et minutes du temps de trajet
        var tempsTrajetSplit = trajet.TempsTrajet.split("h");
        var tempsTrajetHeures = parseInt(tempsTrajetSplit[0], 10); // Conversion en nombre entier
        var tempsTrajetMinutes = parseInt(tempsTrajetSplit[1], 10); // Conversion en nombre entier

        // Calcul du total des minutes de trajet
        totalTempsTrajetMinutes += tempsTrajetHeures * 60 + tempsTrajetMinutes;
        }

        // Convertir les minutes de trajet totales en heures et minutes
        ...
        tableauHtml += "<tr><th colspan='1'>TOTAUX</th><td>"
        + nombreLignes + " matchs</td><td>" 
        + totalDistance + " Km</td><td>" 
        + totalPeages.toFixed(2) + " €</td><td>" 
        + totalHeuresTrajet + "h" + totalMinutesTrajet + "</td><td>" 
        + totalGrandDeplacement.toFixed(2) + " €</td><td>" 
        + totalIndemnitesKilometriques.toFixed(2) + " €</td><td>" 
        + totalRepas.toFixed(2) + " €</td><td>" 
        + totalHotels.toFixed(2) + " €</td><td>"
        + totalPRK.toFixed(2) + " €</td></tr>";

        tableauHtml += "</table>";

        // Calcul du taux horaire
        var tempsTotalMinutes = totalHeuresTrajet * 60 + totalMinutesTrajet; // Conversion du temps total en minutes
        var tauxHoraireEurosParMinute = tempsTotalMinutes > 0 ? (totalIndemnitesKilometriques + indemniteChoisie) / tempsTotalMinutes : 0;

        // Calcul du taux horaire en euros par heure
        var tauxHoraireEurosParHeure = tauxHoraireEurosParMinute * 60; // Conversion du taux horaire en euros par heure 

        // Ajout du taux horaire en euros par heure à la fin du tableau
...
var init = function() {
    ...
    // lance la logique du simulateur
    simu();

    // lance la représentation graphique
    graph();
}